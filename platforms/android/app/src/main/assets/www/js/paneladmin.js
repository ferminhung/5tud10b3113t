            ...
            var aIndicadores = JSON.parse(respuesta);
             $.each( aIndicadores, function( i, value ) {
                ...
                if(parseInt(value["reservas"])>0){
                    $("#menuadmin").html('<center><button onclick="irareservas()" class="btn-burbit-green" ><span class="icon-checkmark"></span></span> '+value["reservas"]+' Asistencias Hoy</button></center>');
                }else{
                    $("#menuadmin").html('<center><button onclick="irareservas()" class="btn-burbit-grey" ><span class="icon-checkmark"></span></span> Sin Asistencia Hoy</button></center>');
                }
                if(parseInt(value["vistaswod"])>0){
                    $("#menuadmin").append('<center><button onclick="iramenuwod()" class="'+value["colorwod"]+'" ><span class="icon-rocket"></span></span> '+value["vistaswod"]+' Vistas de la Cartelera</button></center>');
                }else{
                    $("#menuadmin").append('<center><button onclick="iramenuwod()" class="'+value["colorwod"]+'" ><span class="icon-rocket"></span></span> '+value["vistaswod"]+' Vistas de la Cartelera</button></center>');
                }
                ...
                if(parseInt(value["marcas"])>0){
                    $("#menuadmin").append('<center><button onclick="iramarcas()" class="btn-burbit-green" ><span class="icon-trophy"></span></span> '+value["marcas"]+' Evaluaciones Vistas</button></center>');
                }else{
                    $("#menuadmin").append('<center><button onclick="iramarcas()" class="btn-burbit-grey" ><span class="icon-trophy"></span></span> Sin Evaluaciones Vistas</button></center>');
                }
                if(parseInt(value["totalmegustas"])>0){
                    $("#menuadmin").append('<center><button onclick="vermegustas()" class="btn-burbit-green" ><span class="icon-heart"></span></span> '+value["totalmegustas"]+' Me Gustas en la INFO</button></center>');
                }else{
