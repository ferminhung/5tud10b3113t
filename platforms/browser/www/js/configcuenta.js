$(document).on("pageshow","#configcuenta",function(event, ui){
    ...
    $.ajax({
        ...
        beforeSend:function(){
            ...
            $.mobile.loading( "hide" );
            $("#slcNivel option").remove();
            $("#slcNivel").append('<option value="SIN"> -------- </option>');
            if(respuesta!="SIN REGISTROS"){
                var aAreas = JSON.parse(respuesta);
                $.each( aAreas, function( i, value ) { 
                    $("#slcNivel").append('<option value="'+value['id']+'" ">'+value['strGrado']+'</option>');
                })
            }
	    ...
	    $.ajax({
	        ...
	        beforeSend:function(){
	            ...
	            switch (respuesta['tipo']){
	                case '1':
	                    $("#labNombre").text("Administrador");
	                    break;
	                case '2':
	                    $("#labNombre").text("Estudiante");
	                    break;
	            }
    ...
    $("#btModificar").click(function(){
        ...
        if(sIdentificador!="50104608081"){
            ...
            var sNivel=document.formContacto.slcNivel.value;
            ...
            if (sClave1==sClave2){
                $.ajax({
                    data:{
                        sCodigoWebPhp:sIdentificador, sNombrePhp:sNombre, sEmailPhp:sEmail, sCelPhp:sTelefono, sClavePhp:sClave1, sEstatusPhp:sEstatus, sNivelPhp:sNivel, Mandato:'ModificarCuenta'
                    },
                    url:globalURL,
                    method:'POST',
                    beforeSend:function(){
